...
export async function GET(_request: Request, { params }: { params: Promise<{ formularioId: string }> }) {
    ...
    const capitoli = rows.reduce<Array<{
        id: string;
        titolo: string;
        sortOrder: number;
        argomenti: Array<{ id: string; titolo: string; empty: boolean; sortOrder: number }>;
    }>>((acc, row) => {
        ...
        if (row.argomentoId) {
            ...
            const titleFromHeading = firstLine.startsWith("#") ? firstLine.replace(/^#+\s*/, "").trim() : "";
