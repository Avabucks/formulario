...
export async function GET(request: Request, { params }: { params: Promise<{ formularioId: string }> }) {
    ...
    const { rows, rowCount } = await pool.query(
        `SELECT F.beautiful_id AS "id", F.titolo, F.descrizione, F.owner_uid as "ownerUid", COALESCE(U_A.display_name, 'Utente eliminato') AS "nomeAutore", F.data_creazione as "dataCreazione", F.views, F.visibility,
