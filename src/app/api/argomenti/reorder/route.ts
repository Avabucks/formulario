...
export async function POST(request: Request) {
    ...
    if (!argomentoId) return NextResponse.json({ error: "Argomento obbligatorio" }, { status: 400 });
    if (!targetCapitoloId) return NextResponse.json({ error: "Capitolo obbligatorio" }, { status: 400 });
    if (!Number.isInteger(targetIndex) || targetIndex < 0) return NextResponse.json({ error: "Posizione non valida" }, { status: 400 });
    ...
    try {
        ...
        if (targetRows.length === 0) {
            await client.query("ROLLBACK");
            return NextResponse.json({ error: "Capitolo di destinazione non valido" }, { status: 404 });
        }

        const { rows: sourceArgomenti } = await client.query<{ beautiful_id: string }>(
            `SELECT beautiful_id FROM argomenti WHERE capitolo = $1 ORDER BY sort_order ASC`,
            [sourceCapitoloId]
        );
        const { rows: targetArgomenti } = await client.query<{ beautiful_id: string }>(
            `SELECT beautiful_id FROM argomenti WHERE capitolo = $1 ORDER BY sort_order ASC`,
            [targetCapitoloId]
        );

        const sourceIds = sourceArgomenti.map((row) => row.beautiful_id);
        const targetIds = sourceCapitoloId === targetCapitoloId ? sourceIds : targetArgomenti.map((row) => row.beautiful_id);
        const sourceIndex = sourceIds.indexOf(argomentoId);

        if (sourceIndex === -1) {
            await client.query("ROLLBACK");
            return NextResponse.json({ error: "Argomento non trovato" }, { status: 404 });
        }

        sourceIds.splice(sourceIndex, 1);

        const normalizedTargetIndex = sourceCapitoloId === targetCapitoloId && sourceIndex < targetIndex ? targetIndex - 1 : targetIndex;
        const boundedTargetIndex = Math.max(0, Math.min(normalizedTargetIndex, targetIds.length));
        ...
        if (sourceCapitoloId === targetCapitoloId) {
            ...
            targetWithoutMoved.splice(Math.max(0, Math.min(targetIndex, targetWithoutMoved.length)), 0, argomentoId);
        ...
        if (sourceCapitoloId !== targetCapitoloId) {
            await client.query(`UPDATE argomenti SET capitolo = $1 WHERE beautiful_id = $2`, [targetCapitoloId, argomentoId]);
        ...
        return NextResponse.json({ error: "Errore durante il riordino dell'argomento" }, { status: 500 });
    } finally {
        client.release();
    }
}

async function updateArgomentiOrder(client: PoolClient, capitoloId: string, argomentoIds: string[]) {
