...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (adjacent.length === 0)
      return NextResponse.json(
        { error: "Movimento non possibile" },
        { status: 400 },
      );

    await pool.query(
      `UPDATE argomenti SET sort_order = $1 WHERE beautiful_id = $2`,
      [targetOrder, argomentoId],
    );
    await pool.query(
      `UPDATE argomenti SET sort_order = $1 WHERE beautiful_id = $2`,
      [sort_order, adjacent[0].beautiful_id],
    );
    ...
    return NextResponse.json(
      { error: "Errore nello spostamento dell'argomento" },
      { status: 500 },
    );
  }
}
