...
function saveToHistory(term: string) {
  ...
  localStorage.setItem(
    STORAGE_KEY,
    JSON.stringify([trimmed, ...prev].slice(0, MAX_HISTORY)),
  );
}
...
export function SearchLogic() {
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      if (search.length > 0) {
        Promise.all([
          fetch(
            `/api/formulari/search?titolo=${encodeURIComponent(search)}`,
          ).then((r) => r.json()),
          fetch(
            `/api/capitoli/search?titolo=${encodeURIComponent(search)}`,
          ).then((r) => r.json()),
          fetch(
            `/api/argomenti/search?titolo=${encodeURIComponent(search)}`,
          ).then((r) => r.json()),
  ...
  const hasResults =
    formulari.length > 0 || capitoli.length > 0 || argomenti.length > 0;
        ...
        <Command shouldFilter={false}>
          ...
          <CommandList>
            ...
            {!showHistory && !hasResults && (
              <>
                {loading ? (
                  <CommandEmpty className="flex justify-center items-center gap-2">
                    <Spinner />
              ...
              <CommandGroup heading="Argomenti">
                {argomenti.map((a) => (
                  <CommandItem
                    value={`a-${a.id}`}
                    key={a.id}
                    onSelect={() => handleNavigate(`/editor/${a.id}`)}
                  >
                    <TableOfContents />
                    <div className="flex flex-col justify-center w-full">
                      <span>
