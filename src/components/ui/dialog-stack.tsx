      ...
      value={{
        ...
        setActiveIndex: (value) => {
          if (typeof value === "function") {
            setActiveIndex((prev) => value(prev ?? 0));
          } else {
            setActiveIndex(value);
          }
        },
    ...
    >
      <Portal.Root>
        <div
          ...
          {...props}
        >
          <div className="pointer-events-auto relative grid grid-cols-1 grid-rows-1 w-full place-items-stretch">
...
export const DialogStackContent = ({
  ...
  const scale = 1 - Math.min(Math.abs(distanceFromActive) * 0.035, 0.14);
  const isCurrent = distanceFromActive === 0;
  const isPrevious = distanceFromActive < 0;
