...
export function CapitoloItem({ capitolo }: Readonly<{ capitolo: Capitolo }>) {
    ...
    <>
      ...
      <ItemContent>
        {isEditing ? (
          ...
          <ItemTitle>
            <span className="group-hover:underline">{capitolo.titolo}</span>
          </ItemTitle>
        )}
        {capitolo.argomentiCount !== undefined && !isEditing && (
          <div className="text-xs text-muted-foreground">
            {capitolo.argomentiCount}{" "}
            {capitolo.argomentiCount == 1 ? "argomento" : "argomenti"}
          </div>
        )}
      </ItemContent>
    </>
  ...
  return (
    <Item
      className={`${!isEditing && "hover:bg-muted/50 cursor-pointer"}`}
      variant="outline"
      size="sm"
    >
      <form
      ...
      >
        {isEditing ? (
          ...
          <Link
            href={`/capitolo/${capitolo.id}`}
            onClick={(e: any) => {
              e.preventDefault();
              router.push(`/capitolo/${capitolo.id}`);
            }}
          ...
          <>
            ...
            <Dialog open={deleteOpen} onOpenChange={setDeleteOpen}>
              <DialogContent className="sm:max-w-md">
                ...
                <DialogDescription>
                  Sei sicuro di voler eliminare "{capitolo.titolo}" e tutti i
                  suoi argomenti? Questa azione non è reversibile.
                </DialogDescription>
                <DialogFooter>
                  <DialogClose asChild>
