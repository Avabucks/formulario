...
export function FormularioStructureSection({
  ...
  const [expanded, setExpanded] = useState<Record<string, boolean>>(
    Object.fromEntries(
      structure.capitoli.map((capitolo) => [capitolo.id, true]),
    ),
  );
  ...
  const filteredCapitoli = useMemo(() => {
    ...
    return structure.capitoli
      .map((capitolo) => {
        const capitoloMatches = capitolo.titolo
          .toLowerCase()
      ...
      .filter(
        (capitolo) =>
          capitolo.titolo.toLowerCase().includes(normalizedQuery) ||
  ...
  function handleDrop(capitoloId: string, index: number) {
    ...
    const nextStructure = moveArgomento(
      structure,
      draggedArgomentoId,
      capitoloId,
      index,
  ...
  return (
    <div className="flex min-w-0 flex-col gap-5">
      <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-5">
        ...
        <StructureStat
          label="Argomenti"
          value={structure.stats.argomentiCount}
        />
        <StructureStat
          label="Capitoli vuoti"
          value={structure.stats.emptyCapitoliCount}
          muted
        />
        <StructureStat
          label="Argomenti vuoti"
          value={structure.stats.emptyArgomentiCount}
          muted
        />
        <StructureStat
          label="Senza titolo"
          value={structure.stats.untitledArgomentiCount}
          muted
        />
      </div>
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  />
                  <span className="block w-0 min-w-0 flex-1 truncate font-medium">
                    {capitolo.titolo}
                  </span>
                  <Badge
                    variant={
                      capitolo.argomenti.length === 0 ? "outline" : "secondary"
                    }
                    className="shrink-0"
                  >
                ...
                {isExpanded && (
                  <div
                    className={cn(
                      ...
                      canDrag &&
                        dropTarget?.capitoloId === capitolo.id &&
                        dropTarget.index === capitolo.argomenti.length &&
                        "bg-muted/40",
                    )}
                    onDragOver={(event) => {
                      if (!canDrag) return;
                      ...
                      setDropTarget({
                        ...
                        index: capitolo.argomenti.length,
                      });
                    }}
                    onDrop={() =>
                      handleDrop(capitolo.id, capitolo.argomenti.length)
                    }
                  >
                    {capitolo.argomenti.length === 0 ? (
                      <div className="py-2 pl-7 text-sm text-muted-foreground">
                        Nessun argomento
                      </div>
                    ) : (
...
function moveArgomento(
  structure: FormularioStructure,
  argomentoId: string,
  targetCapitoloId: string,
  targetIndex: number,
) {
  const sourceCapitolo = structure.capitoli.find((capitolo) =>
    capitolo.argomenti.some((argomento) => argomento.id === argomentoId),
  );
  const targetCapitolo = structure.capitoli.find(
    (capitolo) => capitolo.id === targetCapitoloId,
  );

  if (!sourceCapitolo || !targetCapitolo) return structure;

  const movedArgomento = sourceCapitolo.argomenti.find(
    (argomento) => argomento.id === argomentoId,
  );
  if (!movedArgomento) return structure;

  const isSameCapitolo = sourceCapitolo.id === targetCapitoloId;
  const sourceIndex = sourceCapitolo.argomenti.findIndex(
    (argomento) => argomento.id === argomentoId,
  );
  const normalizedTargetIndex =
    isSameCapitolo && sourceIndex < targetIndex ? targetIndex - 1 : targetIndex;
  ...
  const capitoli = structure.capitoli.map((capitolo): StructureCapitolo => {
    if (capitolo.id !== sourceCapitolo.id && capitolo.id !== targetCapitoloId)
      return capitolo;

    const argomenti = capitolo.argomenti.filter(
      (argomento) => argomento.id !== argomentoId,
    );
    ...
    if (capitolo.id === targetCapitoloId) {
      const boundedIndex = Math.max(
        0,
        Math.min(normalizedTargetIndex, argomenti.length),
      );
    ...
    return {
      ...capitolo,
      argomenti: argomenti.map((argomento, index) => ({
        ...argomento,
        sortOrder: index + 1,
      })),
    };
...
function StructureStat({
  ...
  value,
  muted,
}: Readonly<{ label: string; value: number; muted?: boolean }>) {
  return (
